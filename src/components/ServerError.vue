...
<script>
  import {reqdeleteProblem, reqProblemList, reqServerErrorclear, reqServerErrorList} from "../api";
    ...
    export default {
        ...
        data() {
            ...
            clearItems() {
                this.$confirm('此操作将删除所有数据, 是否继续?', '提示', {
                  ...
                  let result = await reqServerErrorclear();
                  if(result.code===200){
                    this.$message({
                      type: "success",
            ...
            deleteItems() {
                this.$confirm('确定删除?', '提示', {
                    ...
                    let url = "deleteServerError";
                    let list = [];
                    this.multipleSelection.forEach((value, index) => list[index] = value.id);
                    list = JSON.stringify(list);
                  let result =await reqdeleteServerError(list);
                  if(result.code===200){
                    this.$message({
                      type: "success",
        ...
        async getList() {
          ...
          let result=await reqServerErrorList(this.currentPage,this.pageSize,{search});
          // console.log(result);
          if(result.code===200){
