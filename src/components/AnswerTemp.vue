    ...
    <el-main>
      <div>
        ...
        <div style="display: flex;align-items:center;">
          ...
          <el-button type="primary" @click="dataRemoval()">数据去重</el-button>
...
<script>
  import {
    ...
    reqAnswerTempList,
    reqAnswerTempmerge,
    reqAnswerTempmergeAll,
    reqAnswerTempModify, reqAnswerTempremoval
  } from "../api";
    ...
    export default {
        ...
        methods: {
           async getList() {
                // let url = "answerTemp/" + this.currentPage + "/" + this.pageSize;
                // let data = this.$qs.stringify({
                //     search: this.search
                // });
             let search=this.search;
             let result=await reqAnswerTempList(this.currentPage,this.pageSize,{search});
             if(result.code===200){
          ...
          deleteSelection() {
                this.$confirm('确定删除?', '提示', {
                    ...
                    if (this.multipleSelection.length !== 0) {
                      let result= await reqAnswerTempDelete(list);
                      if(result.code===200){
                        this.$message({
                          type: "success",
          ...
          answerChanged(form) {
                this.$refs[form].validate(async(valid) => {
                    if (valid) {
                       ...
                       // let url = "answerTemp/modify";
                      this.dialogFormVisible = false;
                      let result= await reqAnswerTempModify(answer);
                      if(result.code===200){
                        this.$message({
                          type: "success",
                ...
                this.$confirm('此操作将删除临时表数据, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async() => {
                    this.loading = true;
                  let result=await reqAnswerTempmergeAll();
                    if(result.code===200){
                      this.$message({
                        type: "success",
                  ...
                  this.loading = false;
                    // this.$axios.post("answerTemp/mergeAll").then((res) => {
                    //
                    // }).catch(() => {
          ...
          dataMerge() {
                this.$confirm('确定要合并所选数据吗?', '提示', {
                    ...
                    if (this.multipleSelection !== 0) {
                     let result=await reqAnswerTempmerge(list);
                     if(result.code===200){
                       this.$message({
                         type: "success",
                         message: "合并了" + result.data + "条数据"
                ...
                this.$confirm('确定要去重数据吗?', '提示', {
                    ...
                    list = JSON.stringify(list);
                    if (this.multipleSelection !== 0) {
                     let result=await reqAnswerTempremoval();
                     if(result.code===200){
                       this.$message({
                         type: "success",
                         message: "去除了" + result.data + "条数据"
                       });
                       this.getList();
                     }else{
                       this.$message.error('出错了！'+result.code+":"+result.data);
